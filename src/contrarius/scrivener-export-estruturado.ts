...
function gerarTimeline(indice: IndiceContrarius, modo: ModoOrdenacaoTimeline, titulo: string, geradoEm: Date): string {
  ...
  linhas.push(`Gerado em: ${geradoEm.toISOString()}`);
  linhas.push('');
  linhas.push('| Ordem | num_reg | Evento | Data | Parte | Livro |');
  linhas.push('| --- | --- | --- | --- | --- | --- |');
  for (const e of linhasEventos) {
    linhas.push(`| ${e[campoUsado] ?? ''} | ${e.num_reg ?? ''} | [${rotuloEvento(e)}](${e.path}) | ${e.data_textual ?? e.data_inicio ?? ''} | ${e.parte?.join(', ') ?? ''} | ${e.livro ?? ''} |`);
  }
  linhas.push('');
